...
inline void ship_ai(GameObject &ship, GameObject &player)
{
    ...
    const auto dir_x = -sin(ship.get_direction() + ALLEGRO_PI); //ship.get_dx(); //-sin(ship.get_direction());
    const auto dir_y = cos(ship.get_direction() + ALLEGRO_PI);  //ship.get_dy(); //cos(ship.get_direction());
...
inline void soldier_ai(GameObject &soldier, GameObject &player)
{
    const auto shooting_dist = fabs(soldier.get_x() - player.get_x()) < 120;
    if (soldier.get_counter(reload_counter_id) == 0)
    {
    ...
    if ((soldier.get_dy() > 0.3 || dx == 0) && soldier.get_counter(ai_soldier_jump_counter_id) == 0)
    {
        ...
        soldier.set_speed(-soldier.get_dx(), -2);
...
inline void tank_ai(GameObject &tank, GameObject &player)
{
    ...
    if (shooting_range)
    {
        const auto dx = player.get_x() - tank.get_x();
        const auto current_dir = tank.get_direction() - (int)(tank.get_direction() / (2 * ALLEGRO_PI)) * 2 * ALLEGRO_PI;
        const auto target_dir = asin(dx / tank.get_distance(player));
